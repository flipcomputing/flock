...
async function setup(page) {
  ...
  await page.waitForFunction(
    () => window.mainWorkspace ?? window.Blockly?.getMainWorkspace?.(),
    { timeout: 20000 },
  );
  await page.waitForTimeout(500); // let initializeBlockHandling attach listeners
}

test("coordinate + colour reporters announce their slot once", async ({
  page,
}) => {
  await setup(page);
  ...
  const out = await page.evaluate(async () => {
    ...
    ch.initSvg(); ch.render();
    const cp = ws.newBlock("colour_picker");
    cp.initSvg(); cp.render();
    ...
    pt.initSvg(); pt.render();
    const dn = ws.newBlock("math_number");
    dn.initSvg(); dn.render();
...
test("decorative block-type icons are silent (no 'empty' in block label)", async ({
  page,
}) => {
  await setup(page);
