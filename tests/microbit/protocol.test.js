  ...
  describe('micro:bit protocol @microbit', function () {
    ...
    describe('serialise', function () {
      ...
      it('serialises scroll text, ASCII-only and truncated', function () {
        ...
        expect(serialiseScrollText('a'.repeat(MAX_SCROLL_TEXT_LENGTH + 10))).to.equal(
          `S:${'a'.repeat(MAX_SCROLL_TEXT_LENGTH)}\n`
        );
      ...
      it('normalises image patterns: pad, truncate, non-digits, non-strings', function () {
        ...
        expect(normaliseImagePattern('9'.repeat(30))).to.equal('9'.repeat(MICROBIT_IMAGE_LENGTH));
        expect(normaliseImagePattern('9x9.9-9 9')).to.equal(
          '909090909' + '0'.repeat(MICROBIT_IMAGE_LENGTH - 9)
        );
        expect(normaliseImagePattern(null)).to.equal('0'.repeat(MICROBIT_IMAGE_LENGTH));
        expect(normaliseImagePattern(undefined)).to.equal('0'.repeat(MICROBIT_IMAGE_LENGTH));
        expect(normaliseImagePattern(42)).to.equal('42' + '0'.repeat(MICROBIT_IMAGE_LENGTH - 2));
    ...
    describe('patchChannelSentinel', function () {
      ...
      it('patches the sentinel digits and fixes the checksum', function () {
        const hex = [makeRecord(0, '00', toBytes('xxFLKCH:001;yzab')), ':00000001FF'].join('\n');
