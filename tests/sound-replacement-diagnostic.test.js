...
export function runSoundReplacementDiagnostic(flock) {
  describe("Sound Replacement Diagnostic @sound @slow @diagnostic", function () {
    ...
    it("should investigate sound replacement timing with loop=false", async function () {
      ...
      console.log(
        "Promise1 created, type:",
        typeof promise1,
        promise1.constructor.name,
      );
      ...
      console.log(
        ...
        mesh.metadata?.currentSound?.name,
      );
      console.log(
        "sound1 === mesh.metadata.currentSound:",
        sound1 === mesh.metadata.currentSound,
      );

      console.log("\n=== Test 2: Playing second sound (loop=false) ===");
      console.log(
        "Before second playSound, currentSound:",
        mesh.metadata?.currentSound?.name,
      );
      ...
      console.log(
        "Promise2 created, type:",
        typeof promise2,
        promise2.constructor.name,
      );
      console.log(
        "Immediately after playSound call, currentSound:",
        mesh.metadata?.currentSound?.name,
      );
      ...
      console.log(
        "After 50ms (promise not awaited), currentSound:",
        mesh.metadata?.currentSound?.name,
      );

      await new Promise((r) => setTimeout(r, 100));
      console.log(
        "After 150ms total (promise not awaited), currentSound:",
        mesh.metadata?.currentSound?.name,
      );

      await new Promise((r) => setTimeout(r, 200));
      console.log(
        "After 350ms total (promise not awaited), currentSound:",
        mesh.metadata?.currentSound?.name,
      );
      ...
      console.log(
        "After promise2 resolved, currentSound:",
        mesh.metadata?.currentSound?.name,
      );
    ...
    it("should investigate sound replacement timing with loop=true", async function () {
      ...
      const mesh = flock.scene.getMeshByName("diagnosticBox");
      console.log(
        "After first sound (loop=true), currentSound:",
        mesh.metadata?.currentSound?.name,
      );
      ...
      console.log(
        "After second sound (loop=true), currentSound:",
        mesh.metadata?.currentSound?.name,
      );
      console.log("sound2:", typeof sound2, sound2?.name);
      console.log(
        "sound2 === mesh.metadata.currentSound:",
        sound2 === mesh.metadata.currentSound,
      );
    ...
    it("should check if playSound deletes currentSound before creating new one", async function () {
      ...
      console.log(
        "Synchronously after playSound call:",
        mesh.metadata?.currentSound?.name,
      );
