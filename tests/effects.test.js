  ...
  describe('Effects API', function () {
    ...
    it('should ignore a particle effect attached to a particle effect', async function () {
      ...
      try {
        ...
        const system = flock.scene.particleSystems.find((s) => s.name === nestedEffectName);
    ...
    it('identical two-color input should match single-color fog behavior', function () {
      ...
      expect(flock._fogAwareShaderMaterials?.has(identicalPairMaterial)).to.not.be.true;
      expect(identicalPairMaterial.fogEnabled).to.equal(singleColorMaterial.fogEnabled);
    ...
    it('startParticleSystem should start a stopped particle system', function () {
      const ps = new flock.BABYLON.ParticleSystem('startTestPS', 100, flock.scene);
    ...
    it('stopParticleSystem should mark the particle system as stopped', function () {
      const ps = new flock.BABYLON.ParticleSystem('stopTestPS', 100, flock.scene);
      ps.emitter = flock.BABYLON.Vector3.Zero();
    ...
    it('resetParticleSystem should clear all particles', function () {
      const ps = new flock.BABYLON.ParticleSystem('resetTestPS', 100, flock.scene);
      ps.emitter = flock.BABYLON.Vector3.Zero();
