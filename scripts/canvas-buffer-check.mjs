...
try {
  ...
  if (healthy.ratio < 3) {
    throw new Error(`Buffer was not device-scaled on a plain run: ${JSON.stringify(healthy)}`);
  }
  ...
  console.log(`Canvas buffer check passed (healthy ${healthy.buf}, repaired ${repaired.buf}).`);
} finally {
