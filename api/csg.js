  ...
  subtractMeshes(modelId, baseMeshName, meshNames) {
    ...
    const nodeInfo = (n, depth = undefined) => {
      ...
      return {
        ...
        uniqueId: n.uniqueId,
        className:
          typeof n.getClassName === "function" ? n.getClassName() : undefined,
        isMesh,
    ...
    const cloneForCSG = (src, name) => {
      ...
      if (dup.isVerticesDataPresent(flock.BABYLON.VertexBuffer.TangentKind))
      ...
      flock.whenModelReady(baseMeshName, (baseMesh) => {
        ...
        flock.prepareMeshes(modelId, meshNames, blockId).then((validMeshes) => {
          ...
          validMeshes.forEach((mesh, meshIndex) => {
            ...
            if (mesh.metadata?.modelName || mesh.getChildren().length > 0) {
              ...
              if (parts.length > 0) {
                ...
                const unified = flock.BABYLON.Mesh.MergeMeshes(
                  partClones,
                  true,
                  true,
                  undefined,
                ...
                if (unified) {
                  ...
                  if (typeof unified.flipFaces === "function")
                    unified.flipFaces();
            ...
            // Simple geometries or meshes that need preparation
              ...
              if (preparedMesh && preparedMesh.geometry) {
                subtractDuplicates.push(
                  cloneForCSG(preparedMesh, `simple_tool_${meshIndex}`),
                );
              }
            }
          });
          ...
          subtractDuplicates.forEach((m, idx) => {
            const meshCSG = tryCSG(`FromMesh(tool[${idx}])`, () =>
