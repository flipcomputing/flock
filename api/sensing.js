  ...
  */
  ...
  getProperty(modelName, propertyName) {
    ...
    const position = (() => {
      ...
      const anchor = typeof flock._getAnchor === 'function' ? flock._getAnchor(mesh) : null;
    ...
    switch (propertyName) {
      ...
      case 'ROTATION_X':
        propertyValue = parseFloat(flock.BABYLON.Tools.ToDegrees(rotEuler.x).toFixed(2));
        break;
      case 'ROTATION_Y':
        propertyValue = parseFloat(flock.BABYLON.Tools.ToDegrees(rotEuler.y).toFixed(2));
        break;
      case 'ROTATION_Z':
        propertyValue = parseFloat(flock.BABYLON.Tools.ToDegrees(rotEuler.z).toFixed(2));
        break;
      ...
      case 'SIZE_X': {
        ...
        propertyValue = parseFloat(
          (bi.boundingBox.maximumWorld.x - bi.boundingBox.minimumWorld.x).toFixed(2)
        );
        break;
      }
      case 'SIZE_Y': {
        ...
        const bi = mesh.getBoundingInfo();
        propertyValue = parseFloat(
          (bi.boundingBox.maximumWorld.y - bi.boundingBox.minimumWorld.y).toFixed(2)
        );
        break;
      }
      case 'SIZE_Z': {
        ...
        const bi = mesh.getBoundingInfo();
        propertyValue = parseFloat(
          (bi.boundingBox.maximumWorld.z - bi.boundingBox.minimumWorld.z).toFixed(2)
        );
        break;
      }
      ...
      case 'SPEED': {
        ...
        const v = body && body._pluginData?.hpBodyId ? body.getLinearVelocity() : null;
        if (!v) {
          propertyValue = 0;
