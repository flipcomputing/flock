...
export function registerDataGenerators(javascriptGenerator) {
  ...
  javascriptGenerator.forBlock['lists_add_item'] = function (block) {
    ...
    const value =
      javascriptGenerator.valueToCode(block, 'TO', javascriptGenerator.ORDER_ASSIGNMENT) || '""';
  ...
  javascriptGenerator.forBlock['lists_delete_nth'] = function (block) {
    ...
    const index =
      javascriptGenerator.valueToCode(block, 'INDEX', javascriptGenerator.ORDER_NONE) || '0';
  ...
  javascriptGenerator.forBlock['lists_getIndex'] = function (block) {
    ...
    const listOrder =
      where === 'RANDOM' ? javascriptGenerator.ORDER_NONE : javascriptGenerator.ORDER_MEMBER;
    ...
    switch (where) {
      ...
      case 'FROM_START': {
        ...
        if (mode === 'GET_REMOVE') {
          return [`${list}.splice(${at}, 1)[0]`, javascriptGenerator.ORDER_FUNCTION_CALL];
        }
        if (mode === 'REMOVE') {
      ...
      case 'FROM_END': {
        ...
        if (mode === 'GET') {
          return [`${list}.slice(${at})[0]`, javascriptGenerator.ORDER_FUNCTION_CALL];
        }
        if (mode === 'GET_REMOVE') {
          return [`${list}.splice(${at}, 1)[0]`, javascriptGenerator.ORDER_FUNCTION_CALL];
        }
  ...
  javascriptGenerator.forBlock['lists_setIndex'] = function (block) {
    ...
    let list =
      javascriptGenerator.valueToCode(block, 'LIST', javascriptGenerator.ORDER_MEMBER) || '[]';
    const mode = block.getFieldValue('MODE') || 'GET';
    let where = block.getFieldValue('WHERE') || 'FROM_START';
    const value =
      javascriptGenerator.valueToCode(block, 'TO', javascriptGenerator.ORDER_ASSIGNMENT) || 'null';
    ...
    switch (where) {
      ...
      case 'FROM_END': {
        ...
        if (mode === 'INSERT') {
          return listCache + `${list}.splice(${list}.length - ${index}, 0, ${value});\n`;
        }
        break;
      }
