...
export function registerAnimateGenerators(javascriptGenerator) {
  ...
  javascriptGenerator.forBlock['switch_animation'] = function (block) {
    ...
    const animationName =
      javascriptGenerator.valueToCode(block, 'ANIMATION_NAME', javascriptGenerator.ORDER_NONE) ||
      '"Idle"';
  ...
  javascriptGenerator.forBlock['glide_to_axis'] = function (block) {
    ...
    const target =
      javascriptGenerator.valueToCode(block, 'TARGET', javascriptGenerator.ORDER_ATOMIC) || '0';
    const duration = getFieldValue(block, 'DURATION', '0');
    const mode = block.getFieldValue('MODE');
    const reverse = block.getFieldValue('REVERSE') === 'TRUE';
    const loop = block.getFieldValue('LOOP') === 'TRUE';
  ...
  javascriptGenerator.forBlock['rotate_to_object'] = function (block) {
    ...
    const apiRotateMode = rotateMode === 'SAME_ROTATION' ? 'same_rotation' : 'towards';
    const duration = getFieldValue(block, 'DURATION', '0');
    const mode = block.getFieldValue('MODE');
    const reverse = block.getFieldValue('REVERSE') === 'TRUE';
    const loop = block.getFieldValue('LOOP') === 'TRUE';
      ...
      for (let i = 0; i < keyframesBlock.inputList.length; i++) {
        ...
        if (valueBlock) {
          ...
          if (valueBlock.type === 'colour_keyframe') {
            ...
            const x =
              javascriptGenerator.valueToCode(valueBlock, 'X', javascriptGenerator.ORDER_ATOMIC) ||
              0;
            const y =
              javascriptGenerator.valueToCode(valueBlock, 'Y', javascriptGenerator.ORDER_ATOMIC) ||
              0;
            const z =
              javascriptGenerator.valueToCode(valueBlock, 'Z', javascriptGenerator.ORDER_ATOMIC) ||
              0;
