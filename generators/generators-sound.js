...
export function registerSoundGenerators(javascriptGenerator) {
  ...
  javascriptGenerator.forBlock['play_theme'] = function (block) {
    ...
    const speedCode =
      javascriptGenerator.valueToCode(block, 'SPEED', javascriptGenerator.ORDER_ATOMIC) || '1';

    const volumeCode =
      javascriptGenerator.valueToCode(block, 'VOLUME', javascriptGenerator.ORDER_ATOMIC) || '1';
  ...
  javascriptGenerator.forBlock['midi_note'] = function (block) {
    const note =
      javascriptGenerator.valueToCode(block, 'NOTE', javascriptGenerator.ORDER_ATOMIC) || '60';
  ...
  javascriptGenerator.forBlock['note'] = function (block) {
    const pitch =
      javascriptGenerator.valueToCode(block, 'PITCH', javascriptGenerator.ORDER_ATOMIC) || '60';
    const duration =
      javascriptGenerator.valueToCode(block, 'DURATION', javascriptGenerator.ORDER_ATOMIC) || '0.5';
    return [`{ pitch: ${pitch}, duration: ${duration} }`, javascriptGenerator.ORDER_ATOMIC];
  };
  ...
  javascriptGenerator.forBlock['play_tune_notes'] = function (block) {
    ...
    const notes =
      javascriptGenerator.valueToCode(block, 'NOTES', javascriptGenerator.ORDER_ATOMIC) || '[]';

    const instrument =
      javascriptGenerator.valueToCode(block, 'INSTRUMENT', javascriptGenerator.ORDER_ATOMIC) ||
  ...
  javascriptGenerator.forBlock['set_music_speed'] = function (block) {
    ...
    const speed =
      javascriptGenerator.valueToCode(block, 'SPEED', javascriptGenerator.ORDER_ATOMIC) || '1.0';
  ...
  javascriptGenerator.forBlock['play_notes'] = function (block) {
    ...
    const notes =
      javascriptGenerator.valueToCode(block, 'NOTES', javascriptGenerator.ORDER_ATOMIC) || '[]';
    const durations =
      javascriptGenerator.valueToCode(block, 'DURATIONS', javascriptGenerator.ORDER_ATOMIC) || '[]';
  ...
  javascriptGenerator.forBlock['create_instrument'] = function (block) {
    ...
    const volume =
      javascriptGenerator.valueToCode(block, 'VOLUME', javascriptGenerator.ORDER_ATOMIC) || '1';
    const effectRate =
      javascriptGenerator.valueToCode(block, 'EFFECT_RATE', javascriptGenerator.ORDER_ATOMIC) ||
      '5';
    const effectDepth =
      javascriptGenerator.valueToCode(block, 'EFFECT_DEPTH', javascriptGenerator.ORDER_ATOMIC) ||
      '0.5';
    const attack =
      javascriptGenerator.valueToCode(block, 'ATTACK', javascriptGenerator.ORDER_ATOMIC) || '0.1';
    const decay =
      javascriptGenerator.valueToCode(block, 'DECAY', javascriptGenerator.ORDER_ATOMIC) || '0.5';
    const sustain =
      javascriptGenerator.valueToCode(block, 'SUSTAIN', javascriptGenerator.ORDER_ATOMIC) || '0.7';
    const release =
  ...
  javascriptGenerator.forBlock['speak'] = function (block) {
    ...
    const rate =
      javascriptGenerator.valueToCode(block, 'RATE', javascriptGenerator.ORDER_ATOMIC) || '1';

    const pitch =
      javascriptGenerator.valueToCode(block, 'PITCH', javascriptGenerator.ORDER_ATOMIC) || '1';
