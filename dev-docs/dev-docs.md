...
- `blocks/transform.js` stores the Blockly definitions for the blocks that handle transformations.
- `create_box` in `blocks/shapes.js` creates a box mesh in the Babylon.js canvas.
- `load_object` in `blocks/models.js` loads a mesh onto the canvas (where the actual game view is).

Each block has an `init` function and this function is _intended_ to contain an event handler (`this.setOnChange((changeEvent) => {...})`) that enacts the change onto the mesh in the canvas itself once it is enacted inside the block. For example:

- the `create_box` block definition in `blocks/shapes.js` calls a `handleBlockChange(block, changeEvent)` method when a modification is detected, whether that be within a field of the block or it simply being added to the existing FlockXR code.

For many blocks, this has already been implemented, and implementing synchronisation for the blocks that still require it is an ongoing work in progress.
...
When a change happens in the _canvas_ editor, the blocks themselves _also_ get updated. [ui/gizmos.js](../ui/gizmos.js) handles all of this live updating through the way of a "gizmo" manager (called `gizmoManager`) that observes changes from the editor and updates the related blocks as they are enacted.
