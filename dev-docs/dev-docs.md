...
Each block has an `init` function and this function is *intended* to contain an event handler (`this.setOnChange((changeEvent) => {...})`) that enacts the change onto the mesh in the canvas itself once it is enacted inside the block. For example:
...
When a change happens in the *canvas* editor, the blocks themselves *also* get updated. [ui/gizmos.js](../ui/gizmos.js) handles all of this live updating through the way of a "gizmo" manager (called `gizmoManager`) that observes changes from the editor and updates the related blocks as they are enacted.
