...
| `<block-icon name="…">`              | A small icon button that lives on a block itself, e.g. `<block-icon name="pin"></block-icon>`. Rendered inline at text size — never paste an SVG for these; add the icon to `BLOCK_ICONS` in `ui/howToPanel.js` first (see the name list below). Unlike `<ui-button>` it is not a link and highlights nothing. |
| `<snippet src="…">caption</snippet>`  | A picture of a real block, e.g. `<snippet src="sky-and-map">The sky block</snippet>`. Rendered live from Blockly JSON in `docs/how-tos/snippets/` — see the README there for how to create one. The caption is optional but recommended: it's also the accessible description, since the rendered picture itself is decorative. Add the `selected` attribute (e.g. `<snippet src="add-star" selected>...</snippet>`) to render it with the same blue selection glow a block gets when picked on the canvas — useful when a step is explaining what "selected" looks like. |
| `<related><related-howto slug="…"></related-howto>…</related>` | A "Related:" box listing other how-tos, styled like `<tip>` (not a chip/pill), e.g. `<related><related-howto slug="position-objects"></related-howto></related>`. Put it wherever it reads best — usually near the end, as a replacement for a `<tip>` that would otherwise just link to another how-to. No label to write: each `<related-howto>` takes only a `slug` and renders that how-to's own title as a plain link, pulled live from `HOW_TOS`/i18n so it can't drift out of sync. |
