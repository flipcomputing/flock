...
<block type="log_variable"></block> <!-- Your custom block -->
 ...
 <category name="Math" colour="${categoryColours["Math"]}">
   ...
   <category name="Functions" custom="PROCEDURE" colour="%{BKY_PROCEDURES_HUE}"></category>
	...
	Blockly.JavaScript.forBlock['show'] = function (block) {
	  const modelName = Blockly.JavaScript.nameDB_.getName(block.getFieldValue('MODEL_VAR'), Blockly.Names.NameType.VARIABLE);
	...
	Blockly.JavaScript.forBlock['hide'] = function (block) {
	  const modelName = Blockly.JavaScript.nameDB_.getName(block.getFieldValue('MODEL_VAR'), Blockly.Names.NameType.VARIABLE);
	...
	function getFieldValue(block, fieldName, defaultValue) {
	  return Blockly.JavaScript.valueToCode(block, fieldName, Blockly.JavaScript.ORDER_ATOMIC) || defaultValue;
	}
	...
	function whenModelReady(meshId, callback, attempt = 1) {
	  ...
	  if (attempt <= maxAttempts) {
		...
		setTimeout(() => whenModelReady(meshId, callback, attempt + 1), attemptInterval);
	  } else {
	...
	Blockly.JavaScript.forBlock['glide_to'] = function (block) {
	  const x = getFieldValue(block, 'X', '0');
	  const y = getFieldValue(block, 'Y', '0');
	  const z = getFieldValue(block, 'Z', '0');
	  const meshName = Blockly.JavaScript.nameDB_.getName(block.getFieldValue('MESH_VAR'), Blockly.Names.NameType.VARIABLE);
	  const duration = block.getFieldValue('DURATION');
...
` : ''}
	...
	Blockly.JavaScript.forBlock['say'] = // Function to handle the 'say' block
	  function (block) {
		const text = Blockly.JavaScript.valueToCode(block, 'TEXT', Blockly.JavaScript.ORDER_ATOMIC) || '""';
		const duration = Blockly.JavaScript.valueToCode(block, 'DURATION', Blockly.JavaScript.ORDER_ATOMIC) || '0';
		const meshVariable = Blockly.JavaScript.nameDB_.getName(block.getFieldValue('MESH_VAR'), Blockly.Names.NameType.VARIABLE);
		const textColor = block.getFieldValue('TEXT_COLOR');
		const backgroundColor = block.getFieldValue('BACKGROUND_COLOR');
		const alpha = Blockly.JavaScript.valueToCode(block, 'ALPHA', Blockly.JavaScript.ORDER_ATOMIC) || '1';
		const size = Blockly.JavaScript.valueToCode(block, 'SIZE', Blockly.JavaScript.ORDER_ATOMIC) || '24';
		const mode = block.getFieldValue('MODE');
			...
			await (async function() {
					...
					return new Promise((resolve, reject) => {
						if (mesh) {
						   ...
						   let plane = mesh.getChildren().find(child => child.name === "textPlane");
							let advancedTexture;
							if (!plane) {
								plane = BABYLON.MeshBuilder.CreatePlane("textPlane", { width: 1.5, height: 1.5 }, scene);
							...
							if (${text}) {
								...
								const bg = new BABYLON.GUI.Rectangle("textBackground");
								bg.background = hexToRgba("${backgroundColor}", ${alpha});
								...
								textBlock.paddingRight = 50;
								 ...
								 textBlock.textVerticalAlignment = BABYLON.GUI.Control.VERTICAL_ALIGNMENT_TOP; // Align text to top
								 textBlock.textHorizontalAlignment = BABYLON.GUI.Control.HORIZONTAL_ALIGNMENT_CENTER; // Align text to left
								...
								console.log(boundingInfo.boundingBox.maximumWorld.y, boundingInfo.boundingBox);
				...
				if ("${asyncMode}" === "AWAIT") {
					await new Promise((resolve, reject) => {
						whenModelReady(${meshVariable}, async function(mesh) {
							try {
					...
					whenModelReady(${meshVariable}, function(mesh) {
						displayText(mesh);
					});
				}
			})();
	...
	Blockly.JavaScript.forBlock['set_fog'] = function (block) {
	  const fogColorHex = block.getFieldValue('FOG_COLOR');
	  const fogMode = block.getFieldValue('FOG_MODE');
	  const fogDensity = Blockly.JavaScript.valueToCode(block, 'FOG_DENSITY', Blockly.JavaScript.ORDER_ATOMIC) || '0.1'; // Default density

	  // Convert hex color to RGB values for Babylon.js
	  const fogColorRgb = `new BABYLON.Color3.FromHexString('${fogColorHex}')`;

	  // Generate the code for setting fog mode
	  let fogModeCode = '';
	  switch (fogMode) {
		case 'NONE':
		  fogModeCode = 'scene.fogMode = BABYLON.Scene.FOGMODE_NONE;\n';
		  break;
		case 'EXP':
		  fogModeCode = 'scene.fogMode = BABYLON.Scene.FOGMODE_EXP;\n';
		  break;
		case 'EXP2':
		  fogModeCode = 'scene.fogMode = BABYLON.Scene.FOGMODE_EXP2;\n';
		  break;
		case 'LINEAR':
		  fogModeCode = 'scene.fogMode = BABYLON.Scene.FOGMODE_LINEAR;\n';
		  break;
	  }

	  return `
	${fogModeCode}
	scene.fogColor = ${fogColorRgb};
	...
	Blockly.JavaScript.forBlock['create_sphere'] = function (block) {
	  ...
	  const posZ = getFieldValue(block, 'Z', '0');
	  const variableName = Blockly.JavaScript.nameDB_.getName(block.getFieldValue('ID_VAR'), Blockly.Names.NameType.VARIABLE);
	...
	Blockly.JavaScript.forBlock['create_plane'] = function (block) {
	  const color = block.getFieldValue('COLOR');
	  const width = getFieldValue(block, 'WIDTH', '1');
	  const height = getFieldValue(block, 'HEIGHT', '1');
	  const posX = getFieldValue(block, 'X', '0');
	  ...
	  let variable_name = Blockly.JavaScript.nameDB_.getName(block.getFieldValue('ID_VAR'), Blockly.Names.NameType.VARIABLE);

	  const planeId = `plane_${generateUUID()}`;
	  meshMap[planeId] = block;

	  return `(function() {
			const newPlane = BABYLON.MeshBuilder.CreatePlane("${planeId}", {width: ${width}, height: ${height}, sideOrientation: BABYLON.Mesh.DOUBLESIDE, scene: scene});
	...
	Blockly.JavaScript.forBlock['set_background_color'] = function (block) {
	  const color = block.getFieldValue('COLOR');
	  return `scene.clearColor = new BABYLON.Color4.FromHexString("${color}FF");\n`;
...
mesh.physics.setMotionType(BABYLON.PhysicsMotionType.ANIMATED);
	...
	Blockly.JavaScript.forBlock['on_each_update'] = function (block) {
	  const branch = Blockly.JavaScript.statementToCode(block, 'DO');
	  return 'scene.onBeforeRenderObservable.add(() => {\n' + branch + '});\n';
	};

	Blockly.JavaScript.forBlock['set_alpha'] = function (block) {
	  const modelName = Blockly.JavaScript.nameDB_.getName(block.getFieldValue('MESH'), Blockly.Names.NameType.VARIABLE);

	  const alphaValue = Blockly.JavaScript.valueToCode(block, 'ALPHA', Blockly.JavaScript.ORDER_ATOMIC);
	...
	Blockly.JavaScript.forBlock['when_clicked'] = function (block) {
	  ...
	  return `
	  whenModelReady(${modelName}, function(_mesh) {
	...
	Blockly.JavaScript.forBlock['tint'] = function (block) {
	  const modelName = Blockly.JavaScript.nameDB_.getName(block.getFieldValue('MODEL_VAR'), Blockly.Names.NameType.VARIABLE);
	  const color = block.getFieldValue('COLOR');
	...
	mesh.overlayColor = new BABYLON.Color3.FromHexString("${color}");
	}
	mesh.getChildMeshes().forEach(function(childMesh) {
	  if (childMesh.material) {
		childMesh.renderOverlay = true;
		childMesh.overlayAlpha = 0.5;
		childMesh.overlayColor = new BABYLON.Color3.FromHexString("${color}");
	...
	Blockly.JavaScript.forBlock['highlight'] = function (block) {
	  const modelName = Blockly.JavaScript.nameDB_.getName(block.getFieldValue('MODEL_VAR'), Blockly.Names.NameType.VARIABLE);
	  const color = block.getFieldValue('COLOR');
	...
	Blockly.JavaScript.forBlock['clear_effects'] = function (block) {
	  const modelName = Blockly.JavaScript.nameDB_.getName(block.getFieldValue('MODEL_VAR'), Blockly.Names.NameType.VARIABLE);
	...
	Blockly.JavaScript.forBlock['move_forward'] = function (block) {
	  ...
	  const modelName = Blockly.JavaScript.nameDB_.getName(block.getFieldValue('MODEL'), Blockly.Names.NameType.VARIABLE);
	  ...
	  return `
		(function() {
		  ...
		  if (model) {
			...
			const cameraForward = scene.activeCamera.getForwardRay().direction.normalize();
	...
	Blockly.JavaScript.forBlock['camera_follow'] = function (block) {
	  const modelName = Blockly.JavaScript.nameDB_.getName(block.getFieldValue('MESH_VAR'), Blockly.Names.NameType.VARIABLE);
...
scene.onAfterPhysicsObservable.add(() => {
  ...
  mesh.physics.setAngularVelocity(BABYLON.Vector3.Zero());
		   ...
		   const camera = new BABYLON.ArcRotateCamera("camera", Math.PI / 2, Math.PI / 4, -20, mesh.position, scene);
	...
	Blockly.JavaScript.forBlock['add_physics'] = function (block) {
	  const modelName = Blockly.JavaScript.nameDB_.getName(block.getFieldValue('MODEL_VAR'), Blockly.Names.NameType.VARIABLE);
	...
	Blockly.JavaScript.forBlock['key_pressed'] = function (block) {
	  const key = block.getFieldValue('KEY');
	  // Code to check if the key is pressed
	  let code;
	  if (key === "ANY") {
		code = 'currentKeyPressed !== null';
	  } else if (key === "NONE") {
		code = 'currentKeyPressed === null';
	  } else {
		code = `currentKeyPressed === "${key}"`;
	  }
	...
	const createScene = function () {
	  ...
	  highlighter = new BABYLON.HighlightLayer("highlighter", scene);
	  ...
	  window.printText = function (text, duration, color) {
		if (text !== '') {
		  ...
		  bg.verticalAlignment = BABYLON.GUI.Control.VERTICAL_ALIGNMENT_TOP;
		  bg.left = "5px"; // Position with some margin from left
		  bg.top = "5px"; // Position with some margin from top


		  // Create a text block
		  const textBlock = new BABYLON.GUI.TextBlock("textBlock", text);
		  ...
		  textBlock.textVerticalAlignment = BABYLON.GUI.Control.VERTICAL_ALIGNMENT_TOP; // Align text to top
		  textBlock.textHorizontalAlignment = BABYLON.GUI.Control.HORIZONTAL_ALIGNMENT_LEFT; // Align text to left
		  textBlock.textWrapping = BABYLON.GUI.TextWrapping.WordWrap;
		  textBlock.resizeToFit = true;
		  textBlock.forceResizeWidth = true;
