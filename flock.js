	...
	animateKeyFrames2(
		...
		return new Promise(async (resolve) => {
			await flock.whenModelReady(meshName, async (mesh) => {
				...
				// Adjusted keyframe generation with seconds-to-frames conversion
				let currentFrame = 0;
				const forwardKeyframes = keyframes.map((keyframe) => {
				...
				// Log generated keyframes for debugging
				console.log("Generated Keyframes: ", forwardKeyframes);

				keyframeAnimation.setKeys(forwardKeyframes);
				...
				const lastFrame = forwardKeyframes[forwardKeyframes.length - 1].frame;
