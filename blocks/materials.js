    ...
    init: function () {
      ...
      const updateInvalidWarning = (block, value) => {
        ...
        block.setWarningText(
          isValidColourInput(value) ? null : translate('colour_from_string_invalid')
        );
      };

      colorField.setValidator(function (newVal) {
        const normalizedInput = typeof newVal === 'string' ? newVal.trim().replace(/^#/, '') : '';
        try {
          const validatedVal = flock.getColorFromString(normalizedInput) || '#000000';
  ...
  function attachSetMaterialOnChange(block) {
    ...
    block.setOnChange((changeEvent) => {
      ...
      const relevant =
        ...
        (changeEvent.type === Blockly.Events.BLOCK_CREATE && changeEvent.blockId === block.id) ||
