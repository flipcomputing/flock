...
export function makeFolderIcon(color, collapsed = false) {
  const icon = collapsed ? ICON_PATHS.folder : ICON_PATHS.folderOpen;
...
export function updateBlockIcons(workspace, iconColor) {
  ...
  for (const block of blocks) {
    ...
    if (iconField) {
      ...
      if (maker) {
        iconField.setValue(
          block.type === 'folder' ? maker(iconColor, block.folderCollapsed_) : maker(iconColor)
        );
      }
    }
  }
