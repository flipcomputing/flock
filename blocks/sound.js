...
class FieldAbcInput extends Blockly.FieldTextInput {
  showEditor_() {
    ...
    hint.textContent = 'Paste or type ABC notation. Ctrl+Enter to import, Escape to cancel.';
...
function parseAbc(abcText) {
  ...
  for (const line of lines) {
    ...
    if (hm) {
      ...
      if (key === 'T') title = v;
        ...
        keyAcc = Object.hasOwn(ABC_KEY_ACCIDENTALS, keyName) ? ABC_KEY_ACCIDENTALS[keyName] : {};
      }
    } else {
  ...
  const melody =
    melodyVoice === null ? musicLines : musicLines.filter((l) => l.voice === melodyVoice);
      ...
      if (c === '|') {
        ...
        if (text[i + 1] === '|') {
          ...
          if (text[i] === '[' && (text[i + 1] === '1' || text[i + 1] === '2')) {
            tokens.push({ t: text[i + 1] === '1' ? 'v1' : 'v2' });
            i += 2;
          } else if (text[i] === '1') {
            tokens.push({ t: 'v1' });
            i++;
  ...
  validBars.forEach((bar, barIdx) => {
    ...
    bar.forEach((note, noteIdx) => {
      ...
      if (note.midi === null) {
        ...
        noteBlock
          .getInput('PITCH')
          .connection.connect(makeNumBlock(ws, note.midi).outputConnection);
      }
  ...
  Blockly.Blocks['play_tune_notes'] = {
    init: function () {
      ...
      this.setOnChange(function (changeEvent) {
        ...
        if (changeEvent.oldParentId !== this.id || changeEvent.oldInputName !== 'NOTES') return;
        ...
        if (notesConn && listBlock?.outputConnection) notesConn.connect(listBlock.outputConnection);
      });
    },
  };
