...
export class FieldMicrobitImage extends Blockly.Field {
  ...
  showEditor_() {
    ...
    Blockly.DropDownDiv.showPositionedByField(this, this._disposeEditor.bind(this));
  ...
  _createEditor() {
    ...
    for (const name of ['clear', 'full']) {
      presetRow.appendChild(this._createPresetButton(name, MICROBIT_IMAGE_PRESETS[name]));
    }
  ...
  _createPictureMenu() {
    ...
    for (const name of MICROBIT_IMAGE_PICTURE_NAMES) {
      ...
      option.type = 'button';
      option.setAttribute('aria-label', translate(`microbit_image_preset_${name}`));
  ...
  _createPresetButton(name, pattern) {
    ...
    button.type = 'button';
    button.setAttribute('aria-label', translate(`microbit_image_preset_${name}`));
  ...
  _createGrid() {
    ...
    grid.addEventListener('pointerdown', (event) => {
      ...
      const { pattern, digit } = toggleImageCell(this.getValue(), cell.row, cell.col);
    ...
    grid.addEventListener('pointermove', (event) => {
      ...
      const cell = this._cellAt(event);
      if (!cell) return;
      this.setValue(setImageCell(this.getValue(), cell.row, cell.col, this._paintDigit));
      this._refreshCells();
    });
    const endPaint = () => {
      this._paintDigit = null;
    ...
    for (const button of this._cellButtons) {
      ...
      button.setAttribute(
        ...
        translate('microbit_image_cell_label')
          ...
          .replace('%3', translate(on ? 'microbit_image_cell_on' : 'microbit_image_cell_off'))
      );
    }
  }
