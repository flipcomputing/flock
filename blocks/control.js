...
export function defineControlBlocks() {
  ...
  Blockly.Blocks["if_clause"] = {
    ...
    updateShape_: function (mode) {
      ...
      if (!wantsCond) {
        const attached = condInput.connection?.targetBlock();
        if (attached) {
          // Save state for restoring later
          this._stashedCondState = Blockly.serialization.blocks.save(attached, {
            addInputBlocks: true,
            addNextBlocks: true,
        ...
        const alreadyHasCond = !!condInput.connection?.targetBlock();
        if (!alreadyHasCond && this._stashedCondState) {
          ...
          if (restored?.outputConnection && condInput.connection) {
            restored.outputConnection.connect(condInput.connection);
          }
